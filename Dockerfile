...
#add WP functionality in php.ini
...
RUN sed -r -e 's/\r$//' /usr/src/wordpress/wp-config-sample.php \
        ...
        | awk '/^\/\*.*stop editing.*\*\/$/ { print("define( '"'"'WP_MEMORY_LIMIT'"'"', '"'"'256M'"'"' );\ndefine( '"'"'MYCRED_DEFAULT_LABEL'"'"', '"'"'Tetoka'"'"' );\ndefine( '"'"'SHOW_MYCRED_IN_WOOCOMMERCE'"'"', true );")}{ print }'> temp.php \

        && chown -R www-data:www-data /usr/src/wordpress/wp-config-sample.php temp.php \
